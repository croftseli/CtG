...
export default function Navbar() {
  ...
  return (
    <>
      <nav aria-label="Main navigation" className="w-full flex items-center justify-between px-6 md:px-12 py-4 md:py-6 fixed top-0 left-0 right-0 z-50 bg-[#FFF4EB]">
            ...
            return (
              <li key={link.href}>
                <Link
                  ...
                  className={`transition-colors hover:text-[#7A5139] ${isActive ? "text-[#C18227] font-semibold border-b-2 border-[#C18227] pb-0.5" : ""
                    }`}
                >
                  {link.label}
                </Link>
        ...
        >
          <Image src="/bell.svg" alt="" width={12} height={12} />
          <span>Notify Me</span>
        </Link>
        ...
        <div
          id="mobile-menu"
          aria-hidden={!isOpen}
          className={`absolute top-[72px] left-0 right-0 bg-[#FFF4EB] shadow-lg md:hidden z-50 overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}
        >
                ...
                return (
                  <li key={link.href} className="border-b">
                    <Link
                      href={link.href}
                      aria-current={isActive ? "page" : undefined}
                      className={`block px-6 py-4 transition-colors hover:text-[#7A5139] ${isActive ? "text-[#C18227] font-semibold" : ""
                        }`}
