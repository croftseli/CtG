  ...
  return (
    <>
      <nav className="w-full flex items-center justify-between px-6 md:px-12 py-4 md:py-6 fixed top-0 left-0 right-0 z-50 bg-[#FFF4EB]">
        <Link href="/" className="pl-2 md:pl-4 scale-125">
        <Image src="/logo.svg" alt="Copper to Gold logo" width={128} height={32} className="w-24 md:w-32" />
      </Link>
      ...
      <Link href="https://docs.google.com/forms/d/e/1FAIpQLSepEd-b_GkPt5RGPD3-7Rs3Jvxwi8BztnbiFTU3sRto70I5LQ/viewform" className="hidden md:flex items-center gap-2 px-5 py-2 rounded bg-[#F7B923] pr-6">
      ...
      <div className={`absolute top-[72px] left-0 right-0 bg-[#FFF4EB] shadow-lg md:hidden z-50 overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
        <ul className="flex flex-col text-lg">
          ...
          <li className="p-4">
            <Link href="https://docs.google.com/forms/d/e/1FAIpQLSepEd-b_GkPt5RGPD3-7Rs3Jvxwi8BztnbiFTU3sRto70I5LQ/viewform" className="flex items-center justify-center gap-2 px-4 py-2 rounded bg-[#F7B923]" onClick={() => setIsOpen(false)}>
              <Image src="/bell.svg" alt="bell" width={12} height={12} />
              <span>Notify Me</span>
            </Link>
          </li>
